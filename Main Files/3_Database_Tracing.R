...
## ---------------------------------- ## 
# Forward Geocoding - Extracting locations 
## ---------------------------------- ## 
# Ending Node-IDs to look for

# 1) Addresses of entities connected to matched officers
nodes_1 <- node_relationships %>% 
  ungroup() %>% 
  filter(origin_match %in% c("1_node_start")) %>% # Origin 1: Officers matches, node start
  dplyr::select(node_id_start, node_id_end, origin_match)

# 2) Potential registered addresses of matched entities 
nodes_2 <- node_relationships %>% 
  ungroup() %>% 
  filter(origin_match %in% c("2_node_start")) %>% # Origin 2: Entities matches, node start
  dplyr::select(node_id_start, node_id_end, origin_match)

# 1_node_start -> Addresses of entities connected to matched officers 
# 2_node_start -> Addresses of matched entities (companies)

# 1) Nodes-addresses: Addresses connected to matched officers (linked entities)
# These are officer-addresses 
address_relations_1 <- nodes_addresses %>% 
  filter(node_id %in% nodes_1$node_id_end) %>% 
  select(node_id, address, name, countries, country_codes, sourceID) %>% 
  mutate(origin_match = "1_node_start")

# 2) Nodes-entities: Addresses of entities connected to officers
# These are entities-addresses 
address_relations_2 <- nodes_entities %>% 
  filter(node_id %in% nodes_1$node_id_end) %>% 
  select(node_id, address, name, countries, country_codes, sourceID) %>% 
  mutate(origin_match = "1_node_start")

# 3) Nodes-addresses: Addresses connected to entities 
address_relations_3 <- nodes_addresses %>% 
  filter(node_id %in% nodes_2$node_id_end) %>% 
  select(node_id, address, name, countries, country_codes, sourceID) %>% 
  mutate(origin_match = "2_node_start")

# 4) Nodes-entities: Addresses connected to matched entities 
address_relations_4 <- nodes_entities %>% 
  filter(node_id %in% nodes_2$node_id_end) %>% 
  select(node_id, address, name, countries, country_codes, sourceID) %>% 
  mutate(origin_match = "2_node_start")

# Merge 1 and 3 to one df
address_relations <- rbind(address_relations_1, address_relations_3)

#### NB NB NB  ####
# Currently, we have only looked at address-nodes. However, it seems like we have to look at addresses from nodes-entities as well.
# The address-nodes are only the addresses directly connected to our matches, while addresses in nodes-entities is the address of a company 
# If we filter for node_ends where our officers is node_start, we thus retrieve the address of a connected company.. #

# Formatting addresses from node-addresses NB: Run again after next rbind (do not run loop under this)
address_relations$countries <- gsub("Peru", "Perú", address_relations$countries) 
address_relations$countries <- gsub("Panama", "Panamá", address_relations$countries) 
address_relations$address <- str_replace_all(address_relations$address, '[.#,";,]', " ")
address_relations$address <- str_replace_all(address_relations$address, '["]', " ")
address_relations$address <- str_replace_all(address_relations$address, "[']", " ")
address_relations$address <- str_replace_all(address_relations$address, "[-]", " ")
address_relations$address <- str_squish(address_relations$address)
address_relations$address <- toupper(address_relations$address)

# Loop for replacing NAs with names / or addresses with names that are longer # Do not run this twice
for (i in 1:nrow(address_relations)) {
  
  # Format addresses and names as strings
  address_relations$address[i] <- toString(address_relations$address[i])
  address_relations$name[i] <- toString(address_relations$name[i])
  
  # Check if address is missing 
  if (is.na(address_relations$address[i])) {
    
    # Add name as an address
    address_relations$address[i] <- address_relations$name[i]
  } 
  
  # If length of name is bigger than length of address, replace
  if (nchar(address_relations$name[i]) >= nchar(address_relations$address[i])) {
    
    # Replace
    address_relations$address[i] <- address_relations$name[i]
  }
  
  # Add country-name to address if it is not included
  if (!is.na(address_relations$countries[i])) {
    
    # Extract countrycode
    country_full <- paste(address_relations$countries[i])
    country_abb <- paste(" ", address_relations$country_codes[i], " ")
    
    country <- c(country_full, country_abb) # Vector of country and country-abb
    
    # Add country to address if not mentioned 
    if (!any(grepl(paste(country, collapse = "|"), address_relations$address[i], ignore.case = TRUE))) {
      
      # Add country if true
      address_relations$address[i] <- paste(address_relations$address[i], toupper(country_full), sep = " ")
      
     cat(i, "\n") # For debugging 
    }
  }
  # Skip iteration if address is not missing or address is longer than name
  else {
    next
  }
}
...
# Merge all to one df
address_relations <- rbind(address_relations, address_relations_2, address_relations_4)

# Loop for extracting addresses for NAs
cl <- makeCluster(6)
...
for (i in 1:nrow(address_relations)) {
  ...
  if(is.na(address_relations$address[i])) {
    ...
    # Extract entity-node
    node <- address_relations$node_id[i]
    
    # Extract address_relationship for this node
    address_rel <- relationships %>% 
      filter(node_id_start == node) %>% 
      filter(rel_type == "registered_address")
    
    # Select first 
    address_node <- address_rel$node_id_end[1]
    
    # Add address if it exists
    if (!is_empty(address_node) & !is.na(address_node)) {
      
      # Extract connected address
      address <- nodes_addresses %>% 
        filter(node_id == address_node) %>% 
        select(name)
      
      # Add to df
      address_relations$address[i] <- address$name[1]
...
# Clean a bit
address_relations <- subset(address_relations, !grepl("^B & O BERMUDEZ & OCAMPO", address)) # Remove addresses causing API to stop 
address_relations <- subset(address_relations, !grepl("^SUAREZ & ASOCIADO", address)) # Same here 
address_relations <- address_relations[!is.na(address_relations$address), ] # Remove rows with NAs

write_csv(address_relations, "CSV-files/address_relations")

address_relations <- read.csv("CSV-files/address_relations")
...
locations_geo <- mutate_geocode(address_relations, address, output = "more") # Forward geocoding

toc() # Track time 
stopCluster(cl) # Stop cluster 

# Save results
write_csv(locations_geo, "locations_google2.csv")
